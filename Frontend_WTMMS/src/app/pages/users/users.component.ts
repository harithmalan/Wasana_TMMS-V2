...
export class UsersComponent implements OnInit {
  ...
  readonly roles = ['Admin', 'Manager', 'Operator', 'Sales'];
  ...
  openAdd() {
    ...
    this.form = { role: 'Operator', status: 'Active' };
    this.formErrors = {};
  ...
  save() {
    ...
    if (editing) {
      this.data.updateUser({ ...editing, ...this.form } as User).subscribe({
        ...
        error: (err) => this.toast.show('Failed to update user', 'error')
      });
    } else {
      this.data.addUser(this.form as Omit<User, 'id' | 'lastLogin' | 'avatar'>).subscribe({
        next: () => {
  ...
  getRoleBg(role: string): string {
    const m: Record<string, string> = { Admin: '#fee2e2', Manager: '#dbeafe', Operator: 'var(--secondary)', Sales: '#dcfce7' };
    return m[role] ?? 'var(--secondary)';
  }
  getRoleColor(role: string): string {
    const m: Record<string, string> = { Admin: '#991b1b', Manager: '#1e40af', Operator: 'var(--secondary-foreground)', Sales: '#166534' };
